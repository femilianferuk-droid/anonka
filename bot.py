    ...
    if query.data == "profile":
        ...
        keyboard = [
            [InlineKeyboardButton("🔗 Поделиться ссылкой", 
                                url=f"https://t.me/share/url?url={profile_link}&text=Напиши%20мне%20анонимное%20сообщение!")],
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Обработка старта с параметром (отправка анонимного сообщения)
    if context.args and update.message.text == '/start':
    ...
    # Если пользователь отправляет сообщение целевого пользователя
...
def main():
    ...
    # Обработчики кнопок
    application.add_handler(CallbackQueryHandler(button_handler, pattern="^(profile|help|back_to_main|my_messages)$"))
